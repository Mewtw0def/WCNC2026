...
def plot_prediction(y_tot, y_true, y_pred, epoch, round, pred_len, model_name):
    left_part_seq = []
    for item in y_tot[-1:, :, -1:].tolist()[0]:
        left_part_seq.append(item[0])
    
    right_part_true = []
    for item in y_true[-1:, :, -1:].tolist()[0]:
        right_part_true.append(item[0])

    right_part_pred = []
    for item in y_pred[-1:, :, -1:].tolist()[0]:
        right_part_pred.append(item[0])

    left_part = left_part_seq + right_part_true[:-pred_len]
    right_part = right_part_true[-pred_len:]

    ground_truth = left_part + right_part
    predict = left_part + right_part_pred

    # print(left_part)
    # print(right_part_true)
    # print(right_part_pred)



    plt.figure(figsize=(12, 4))
    plt.plot(ground_truth, label="Ground Truth", linewidth=2)
    plt.plot(predict, label="Prediction", linewidth=2, linestyle='--')
    ...
    folder_path = "./pics/" + model_name 
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

    save_path = folder_path + "/Epoch_" + str(epoch) + "_Round_" + str(round) + ".png"
    if save_path:
